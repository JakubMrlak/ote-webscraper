...
def save_intervals(df: pd.DataFrame, delivery_date: date, session: str) -> Path:
    """Uloží intervalová data do parquet souboru."""
    path = _build_path(OUTPUT_DIR, delivery_date, session)
...
def save_summary(df: pd.DataFrame, delivery_date: date, session: str) -> Path:
    """Uloží souhrnná data do parquet souboru."""
    path = _build_path(SUMMARY_DIR, delivery_date, session)
    path.parent.mkdir(parents=True, exist_ok=True)
    df.to_parquet(path, index=False)
...
def _build_path(base_dir: Path, delivery_date: date, session: str) -> Path:
